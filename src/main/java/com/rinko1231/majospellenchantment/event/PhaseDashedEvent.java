...
public class PhaseDashedEvent {
    ...
    public void onSpellCast(LivingDamageEvent.Post event) {
        if(!MajoSpellEnchantmentConfig.phaseDashedEnabled.get()) return;

        if (!(event.getSource().getEntity() instanceof ServerPlayer player)) return;

        if (!(event.getSource() instanceof SpellDamageSource)) return;
        ItemStack boots = player.getItemBySlot(EquipmentSlot.FEET);
        int phaseLevel = boots.getEnchantmentLevel(
                player.level().registryAccess()
                        .lookupOrThrow(Registries.ENCHANTMENT)
                        .getOrThrow(EnchantmentRegistry.PHASE_DASHED_ENCHANT)
        );
