...
import static com.rinko1231.majospellenchantment.init.EnchantmentRegistry.VLAD_TEPES_ENCHANT;

public class VladTepesEvent {

    @SubscribeEvent
    public void onArrowLoose(ArrowLooseEvent event) {
        if(!MajoSpellEnchantmentConfig.vladTepesEnabled.get()) return;
        ...
        int enchantLevel = EnchantmentHelper.getItemEnchantmentLevel(
                player.level().holderLookup(VLAD_TEPES_ENCHANT.registryKey())
                        .getOrThrow(VLAD_TEPES_ENCHANT),
                bow
        );
        if (enchantLevel <= 0) return;

        AttributeInstance attr = player.getAttribute(AttributeRegistry.BLOOD_SPELL_POWER);
        double bloodPower =0;
        if(attr != null)
